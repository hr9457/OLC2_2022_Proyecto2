...
class Forin(Instruccion):
    ...
    def optimizar(self, entorno, traductor3d, cadena):
        ...
        # utildades para el manejo del for        
        # exp --> n (pivote)
        pivote = self.variable.optimizar(entorno, traductor3d, cadena)
        posicion_pivote = traductor3d.getStack()
        traductor3d.aumentarStack()
        ...
        # expInicio
        inicio_for = self.inicio.optimizar(entorno, traductor3d, cadena)
        if inicio_for.tipo == TipoExpresion.ID:
        ...
        # expFinal
        final_for = self.final.optimizar(entorno, traductor3d, cadena)
        if final_for.tipo == TipoExpresion.ID:
        ...
        for instruccion in self.instrucciones:
            instruccion.optimizar(envFor, traductor3d, cadena)
